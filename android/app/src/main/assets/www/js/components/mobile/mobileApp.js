...
// Manejo global del botón Atrás de Android / Navegador
let historyPopStateAttached = false;
function setupNavigationHistory(rootElement) {
  if (historyPopStateAttached) return;
  historyPopStateAttached = true;

  window.addEventListener('popstate', () => {
    // 1. Si el modal del teclado numérico está abierto, cerrarlo
    const modalRoot = document.getElementById('keypad-modal-root');
    if (modalRoot && modalRoot.children.length > 0) {
      modalRoot.innerHTML = '';
      return;
    }

    // 2. Si está en Facturas o Historial, volver a Inicio
...
function renderMobileAppShell(rootElement, activeUser) {
  ...
  setupNavigationHistory(rootElement);
  ...
  rootElement.innerHTML = `
    <div class="mobile-app-container">
      ...
      <div class="mobile-content-area is-dashboard" id="mobile-tab-content">
        <!-- Render dinámico según pestaña activa -->
      </div>

      <!-- Barra de Botones de Acción Gigantes (Fijos abajo sobre la barra) -->
      <div class="mobile-fixed-bottom-actions" id="mobile-quick-actions-bar">
...
async function renderCurrentTab(rootElement) {
  ...
  try {
    ...
    // Controlar visibilidad de la barra de acciones rápidas (solo en Inicio)
    const quickBar = document.getElementById('mobile-quick-actions-bar');
    if (quickBar) {
      quickBar.style.display = currentTab === 'DASHBOARD' ? 'grid' : 'none';
    }

    if (currentTab === 'DASHBOARD') {
      container.classList.add('is-dashboard');
    } else {
      container.classList.remove('is-dashboard');
    }
    ...
    if (currentTab === 'BILLS') {
      ...
      // DASHBOARD PRINCIPAL DEL USUARIO (Compacto, 1 pantalla fija sin scroll)
      const recentTx = (await getTransactions(activeUser, 'ALL')).slice(0, 2);

      container.innerHTML = `
        <div style="display: flex; flex-direction: column; gap: 10px;">
          <!-- TARJETAS DE SALDO EN CUADRÍCULA DUAL (COMPACTA) -->
          ...
          ${summary.pendingBillsCount > 0 ? `
            <div class="alert-bills-card" id="alert-card-trigger" title="Ver facturas por pagar">
          ...
          ` : `
            <div style="background: rgba(16, 185, 129, 0.08); border: 1px solid rgba(16, 185, 129, 0.2); border-radius: var(--radius-md); padding: 10px 14px; display: flex; align-items: center; gap: 8px; font-size: 12px; color: #34d399;">
              <span>✅</span>
              <span>No hay facturas de servicios pendientes para el hogar.</span>
            </div>
          `}

          <!-- HISTORIAL RÁPIDO DE ÚLTIMOS MOVIMIENTOS (COMPACTO) -->
          <div class="card" style="padding: 12px 14px; margin-top: 2px;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
              <h3 style="font-size: 13px; font-weight: 700; color: var(--text-main); margin: 0;">Tus Últimos Movimientos</h3>
              <button type="button" id="btn-view-all-history" style="background: none; border: none; color: var(--color-digital); font-size: 11px; font-weight: 700; cursor: pointer;">
                Ver Todos →
              </button>
            </div>
            ...
            <div class="tx-list">
              ${recentTx.length === 0 ? `
                <div style="text-align: center; padding: 14px 0; color: var(--text-muted); font-size: 12px;">
              ...
              ` : recentTx.map(tx => {
                ...
                return `
                  <div class="tx-item" style="padding: 6px 0;">
                    <div class="tx-left">
                      <div class="tx-icon ${isIncome ? 'income' : 'expense'}" style="width: 30px; height: 30px; font-size: 13px;">
                        ${isIncome ? '↓' : '↑'}
                      </div>
                      <div class="tx-info">
                        <h4 style="font-size: 13px;">${tx.category}</h4>
                    ...
                    <div class="tx-amount ${isIncome ? 'income' : 'expense'}" style="font-size: 14px;">
      ...
      document.getElementById('alert-card-trigger')?.addEventListener('click', () => switchTab('BILLS', rootElement));
      document.getElementById('btn-view-all-history')?.addEventListener('click', () => switchTab('HISTORY', rootElement));
    }
  } catch (err) {
