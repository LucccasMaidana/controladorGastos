...
import { registerIncome, registerExpense, formatCurrency } from '../../services/accounting.js?v=13';
...
export function openKeypadModal(mode = 'INCOME', userName = 'Usuario', onClose = null) {
  ...
  try {
    history.pushState({ modal: 'keypad' }, '');
  } catch (e) {}

  renderKeypadModal();
}

export function closeKeypadModal(fromPopState = false) {
  currentModalState.isOpen = false;
  const container = document.getElementById('keypad-modal-root');
  if (container) {
    container.innerHTML = '';
  }
  if (!fromPopState && history.state && history.state.modal === 'keypad') {
    try {
      history.back();
    } catch (e) {}
  }
