...
async function startApp() {
  ...
  try {
    ...
    // 0.1 Limpieza única y definitiva de versiones previas en este navegador
    if (!localStorage.getItem('libreta_migrated_v3')) {
      console.log('🧹 Primera carga de v3: Purgando datos viejos de prueba...');
      await clearAllLocalData();
      localStorage.removeItem('libreta_active_user');
      localStorage.setItem('libreta_migrated_v3', 'true');
    }
    ...
    // 5.1 Si viene en la app nativa (APK), o con ?mode=admin o ?mode=mobile, o pantalla de celular
    const urlParams = new URLSearchParams(window.location.search);
    const mode = (urlParams.get('mode') || urlParams.get('view') || '').toLowerCase();
    const isNativeApp = navigator.userAgent.includes('LibretaContableNative') || window.isNativeAndroidApp === true;
    const isMobileDevice = window.innerWidth <= 768;

    if (mode === 'admin') {
      document.body.classList.add('standalone-admin');
    } else if (isNativeApp || mode === 'mobile' || urlParams.has('mariel') || (isMobileDevice && !mode)) {
