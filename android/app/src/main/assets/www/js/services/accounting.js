 ...
 */
export async function createBill({ serviceName, amount, dueDate, secondDueDate = null, secondAmount = null, createdBy = 'Admin' }) {
  const validAmount = roundCurrency(amount);
  ...
  const bill = {
    ...
    amount: validAmount,
    due_date: dueDate,
    second_due_date: secondDueDate || null,
    second_amount: (secondAmount !== null && secondAmount !== undefined && !isNaN(secondAmount) && Number(secondAmount) > 0) ? roundCurrency(secondAmount) : null,
    status: 'PENDING',
    paid_at: null,
...
/**
 * 9. OBTENER ESTILO VISUAL E IDENTIDAD DE MARCA POR SERVICIO
 * AySA, Edenor, Naturgy, Movistar, Mercado Pago y Estándar
 */
export function getServiceBrandStyle(serviceName = '') {
  const s = (serviceName || '').toLowerCase().trim();

  // AySA (Agua): Azul Marino (#003865 / #004b87)
  if (s.includes('aysa') || s.includes('agua') || s.includes('sanitario')) {
    return {
      brand: 'AySA',
      brandKey: 'brand-aysa',
      icon: '💧',
      borderColor: '#004b87',
      bgBadge: 'rgba(0, 75, 135, 0.25)',
      colorBadge: '#7dd3fc',
      gradient: 'linear-gradient(135deg, #002855 0%, #004b87 100%)',
      lightBg: '#f0f9ff',
      lightBorder: '#0284c7',
      lightText: '#0369a1'
    };
  ...
  // Edenor: Azul con gradiente a verde (#00549f -> #78be20)
  if (s.includes('edenor') || s.includes('luz') || s.includes('electric') || s.includes('edesur')) {
    return {
      brand: 'Edenor',
      brandKey: 'brand-edenor',
      icon: '💡',
      borderColor: '#78be20',
      bgBadge: 'rgba(120, 190, 32, 0.25)',
      colorBadge: '#bef264',
      gradient: 'linear-gradient(135deg, #00549f 0%, #78be20 100%)',
      lightBg: '#f7fee7',
      lightBorder: '#84cc16',
      lightText: '#3f6212'
    };
  ...
  // Naturgy: Naranja (#f26b21 / #eb6100)
  if (s.includes('naturgy') || s.includes('gas') || s.includes('metrogas')) {
    return {
      brand: 'Naturgy',
      brandKey: 'brand-naturgy',
      icon: '🔥',
      borderColor: '#f26b21',
      bgBadge: 'rgba(242, 107, 33, 0.22)',
      colorBadge: '#fdba74',
      gradient: 'linear-gradient(135deg, #ea580c 0%, #f26b21 100%)',
      lightBg: '#fff7ed',
      lightBorder: '#f97316',
      lightText: '#c2410c'
    };
  ...
  // Movistar: Verde Movistar (#5bc500 / #00a9e0)
  if (s.includes('movistar') || s.includes('internet') || s.includes('wifi') || s.includes('telef') || s.includes('fibertel') || s.includes('telecom') || s.includes('claro') || s.includes('personal')) {
    return {
      brand: 'Movistar',
      brandKey: 'brand-movistar',
      icon: '📱',
      borderColor: '#5bc500',
      bgBadge: 'rgba(91, 197, 0, 0.22)',
      colorBadge: '#86efac',
      gradient: 'linear-gradient(135deg, #00a9e0 0%, #5bc500 100%)',
      lightBg: '#f0fdf4',
      lightBorder: '#22c55e',
      lightText: '#15803d'
    };
  ...
  // Mercado Pago: Amarillo (#ffe600 / #f5af00) con texto oscuro para contraste óptimo
  if (s.includes('mercadopago') || s.includes('mercado pago') || s.includes('mp') || s.includes('mercado libre')) {
    return {
      brand: 'Mercado Pago',
      brandKey: 'brand-mercadopago',
      icon: '💳',
      borderColor: '#ffe600',
      bgBadge: '#ffe600',
      colorBadge: '#0a192f',
      gradient: 'linear-gradient(135deg, #ffe600 0%, #f5af00 100%)',
      lightBg: '#fefce8',
      lightBorder: '#eab308',
      lightText: '#854d0e'
    };
  ...
  // Estándar / Neutro (Municipalidad, ARBA, Expensas, etc.)
  const isExpensa = s.includes('expensa');
  const isGov = s.includes('arba') || s.includes('muni') || s.includes('abl');
  return {
    brand: 'Servicio',
    brandKey: 'brand-standard',
    icon: isExpensa ? '🏢' : (isGov ? '🏛️' : '📑'),
    borderColor: 'var(--border-medium)',
    bgBadge: 'var(--bg-surface-elevated)',
    colorBadge: 'var(--text-main)',
